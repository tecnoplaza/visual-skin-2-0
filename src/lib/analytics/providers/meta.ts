...
export function loadMeta(setting: AnalyticsSetting) {
  ...
  const fbq: any = (...args:any[]) => { if (fbq.callMethod) fbq.callMethod(...args); else fbq.queue.push(args); };
  fbq.push=fbq; fbq.queue=[]; fbq.loaded=true; fbq.version="2.0"; window.fbq=fbq; window._fbq=fbq;
