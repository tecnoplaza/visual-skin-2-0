...
import { hasPendingSensitiveToken } from "@/lib/analytics/privacy";
...
export default function AnalyticsManager(){
  ...
  useEffect(()=>{setOpen(!getConsent());const show=()=>{setCustom(true);setOpen(true)};const changed=()=>{if(!hasPendingSensitiveToken(window.location.search))void initializeAnalyticsProviders()};window.addEventListener("visualskin:open-consent",show);window.addEventListener("visualskin:consent",changed);return()=>{window.removeEventListener("visualskin:open-consent",show);window.removeEventListener("visualskin:consent",changed)}},[]);
  useEffect(()=>{if(hasPendingSensitiveToken(location.searchStr))return;void initializeAnalyticsProviders();if(lastPageViewPath.current===location.pathname)return;lastPageViewPath.current=location.pathname;trackVisualSkinEvent({event_name:"page_view"})},[location.pathname,location.searchStr]);
  if(!open)return null;
