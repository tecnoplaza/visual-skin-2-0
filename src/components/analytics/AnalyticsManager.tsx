...
export default function AnalyticsManager(){
  ...
  useEffect(()=>{setOpen(!getConsent());const show=()=>{setCustom(true);setOpen(true)};const changed=()=>{if(!new URLSearchParams(window.location.search).has("token"))void initializeAnalyticsProviders()};window.addEventListener("visualskin:open-consent",show);window.addEventListener("visualskin:consent",changed);return()=>{window.removeEventListener("visualskin:open-consent",show);window.removeEventListener("visualskin:consent",changed)}},[]);
  useEffect(()=>{const query=new URLSearchParams(location.searchStr);if(query.has("token"))return;void initializeAnalyticsProviders();trackVisualSkinEvent({event_name:"page_view"})},[location.pathname,location.searchStr]);
  if(!open)return null;
