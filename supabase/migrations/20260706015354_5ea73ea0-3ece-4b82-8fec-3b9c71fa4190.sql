CREATE OR REPLACE FUNCTION public.apply_mercado_pago_payment_response(
  p_order_id uuid, p_attempt_id uuid, p_payment_id text, p_payment_status text,
  p_status_detail text, p_live_mode boolean, p_transaction_amount numeric,
  p_currency_id text, p_external_reference text, p_metadata_order_id text,
  p_metadata_attempt_id text, p_payment_type_id text, p_collector_id text,
...
BEGIN
  ...
  -- Validaciones canónicas. Cualquier mismatch => reconciliación.
  -- NOTA: p_live_mode se almacena como dato de auditoría en status_detail
  -- pero NO se usa como guard: MP puede devolver live_mode=true incluso con
  -- credenciales de test-user. La separación test/producción está garantizada
  -- por (a) las credenciales usadas para consultar el pago y (b) la
  -- coincidencia entre v_attempt.payment_environment y v_order.payment_environment.
